...
#ifndef MEM_SYSTEM_XSPDBP_H
#define	MEM_SYSTEM_XSPDBP_H
...
typedef struct cache_list_head_xspdbp_t
{
  ub4 rrpv;
  struct cache_block_t *head;
}xspdbp_list;

#define XSPDBP_DATA_MAX_RRPV(data)   ((data)->max_rrpv)
#define XSPDBP_DATA_THRESHOLD(data)  ((data)->threshold)
#define XSPDBP_DATA_BLOCKS(data)     ((data)->blocks)
#define XSPDBP_DATA_VALID_HEAD(data) ((data)->valid_head)
#define XSPDBP_DATA_VALID_TAIL(data) ((data)->valid_tail)
#define XSPDBP_DATA_FREE_HEAD(data)  ((data)->free_head)
#define XSPDBP_DATA_FREE_TAIL(data)  ((data)->free_tail)

/* RRIP specific data */
typedef struct cache_policy_xspdbp_data_t
{
  cache_policy_t  following;                    /* Policy set is following */

  ub4        max_rrpv;                          /* Maximum RRPV */
  ub4        threshold;                         /* Eviction threshold */
  xspdbp_list *valid_head;                        /* Valid block head */
  xspdbp_list *valid_tail;                        /* Valid block tail */
...
typedef struct cache_policy_xspdbp_gdata_t
{
  /* Eight counter to be used for XSPDBP reuse probability learning */
...
void cache_init_xspdbp(long long int set_indx, struct cache_params *params, 
  xspdbp_data *policy_data, xspdbp_gdata *global_data);
...
void cache_access_block_xspdbp(xspdbp_data *policy_data, xspdbp_gdata *global_data,
...
struct cache_block_t cache_get_block_xspdbp(xspdbp_data *policy_data, xspdbp_gdata *global_data, 
...
int cache_get_new_rrpv_xspdbp(xspdbp_data *policy_data, xspdbp_gdata *global_data, 
  int way, int strm);
 ...
 * DESCRIPTION
 *  
 *  Update XSPDBP spesific fill counters 
 *
 * PARAMETERS
 *  
 *  policy_data (IN)  - Cache Set
 *  global_data (OUT) - XSPDBP specific cache wide data
 *  way         (IN)  - Block way
 *  strm        (IN)  - Access stream
 *
 * RETURNS
...
void cache_update_fill_counter_xspdbp(srrip_data *policy_data, 
  xspdbp_gdata *global_data, int way, int strm);
 ...
 * DESCRIPTION
 *  
 *  Update XSPDBP specific hit counter 
 *
 * PARAMETERS
 *  
 *  policy_data (IN)  - Cache Set
...
void cache_update_hit_counter_xspdbp(srrip_data *policy_data,
  xspdbp_gdata *global_data, int way, int strm);
