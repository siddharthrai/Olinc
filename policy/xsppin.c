...
#define CACHE_XSPPIN_INCREMENT_RRPV(head_ptr, tail_ptr, rrpv)\
do                                                            \
{                                                             \
    int dif = 0;                                              \
                                                              \
...
{                                                                                           \
        ...
        assert(XSPPIN_DATA_FREE_HEAD(set) && XSPPIN_DATA_FREE_HEAD(set));                     \
                                                                                            \
        /* Check: current block must be in invalid state */                                 \
        assert((blk)->state == cache_block_invalid);                                        \
                                                                                            \
        CACHE_REMOVE_FROM_SQUEUE(blk, XSPPIN_DATA_FREE_HEAD(set), XSPPIN_DATA_FREE_TAIL(set));    \
                                                                                            \
        (blk)->next = NULL;                                                                 \
        (blk)->prev = NULL;                                                                 \
                                                                                            \
...
int xsppin_cs_blocks = 0;

void cache_init_xsppin(int set_idx, struct cache_params *params, xsppin_data *policy_data,
...
#define MEM_ALLOC(size) ((rrip_list *)xcalloc(size, sizeof(rrip_list)))
  ...
  XSPPIN_DATA_VALID_HEAD(policy_data) = MEM_ALLOC(MAX_RRPV + 1);
  XSPPIN_DATA_VALID_TAIL(policy_data) = MEM_ALLOC(MAX_RRPV + 1);

#undef MEM_ALLOC  

  assert(XSPPIN_DATA_VALID_HEAD(policy_data));
  assert(XSPPIN_DATA_VALID_TAIL(policy_data));

  /* Set max RRPV for the set */
  XSPPIN_DATA_MAX_RRPV(policy_data)  = MAX_RRPV;
  XSPPIN_DATA_SET_INDEX(policy_data) = set_idx;

  /* Ways for the set */
  XSPPIN_DATA_WAYS(policy_data) = params->ways;

  /* Initialize head nodes */
  for (int i = 0; i <= MAX_RRPV; i++)
  {
    XSPPIN_DATA_VALID_HEAD(policy_data)[i].rrpv = i;
    XSPPIN_DATA_VALID_HEAD(policy_data)[i].head = NULL;
    XSPPIN_DATA_VALID_TAIL(policy_data)[i].head = NULL;
  }

  /* Create array of blocks */
  XSPPIN_DATA_BLOCKS(policy_data) = 
...
#define MEM_ALLOC(size) ((list_head_t *)xcalloc(size, sizeof(list_head_t)))
  ...
  XSPPIN_DATA_FREE_HLST(policy_data) = MEM_ALLOC(1);
  assert(XSPPIN_DATA_FREE_HLST(policy_data));

  XSPPIN_DATA_FREE_TLST(policy_data) = MEM_ALLOC(1);
  assert(XSPPIN_DATA_FREE_TLST(policy_data));

#undef MEM_ALLOC

  /* Initialize array of blocks */
  XSPPIN_DATA_FREE_HEAD(policy_data) = &(XSPPIN_DATA_BLOCKS(policy_data)[0]);
  XSPPIN_DATA_FREE_TAIL(policy_data) = &(XSPPIN_DATA_BLOCKS(policy_data)[params->ways - 1]);

  for (int way = 0; way < params->ways; way++)
  {
    (&XSPPIN_DATA_BLOCKS(policy_data)[way])->way   = way;
    (&XSPPIN_DATA_BLOCKS(policy_data)[way])->state = cache_block_invalid;
    (&XSPPIN_DATA_BLOCKS(policy_data)[way])->next  = way ?
      (&XSPPIN_DATA_BLOCKS(policy_data)[way - 1]) : NULL;
    (&XSPPIN_DATA_BLOCKS(policy_data)[way])->prev  = way < params->ways - 1 ? 
      (&XSPPIN_DATA_BLOCKS(policy_data)[way + 1]) : NULL;
  }
  
  /* Set current and default fill policy to SRRIP */
  XSPPIN_DATA_CFPOLICY(policy_data) = cache_policy_xsppin;
  XSPPIN_DATA_DFPOLICY(policy_data) = cache_policy_xsppin;
  XSPPIN_DATA_CAPOLICY(policy_data) = cache_policy_xsppin;
  XSPPIN_DATA_DAPOLICY(policy_data) = cache_policy_xsppin;
  XSPPIN_DATA_CRPOLICY(policy_data) = cache_policy_xsppin;
  XSPPIN_DATA_DRPOLICY(policy_data) = cache_policy_xsppin;
  
  assert(XSPPIN_DATA_MAX_RRPV(policy_data) != 0);
  ...
  if (set_idx == 0)
  {
    ...
    for (ub4 i = 0; i < TST; i++)
    {
      global_data->per_stream_frrpv[i] = xcalloc(1, sizeof(ub8) * XSPPIN_DATA_MAX_RRPV(policy_data));
...
void cache_free_xsppin(xsppin_data *policy_data)
{
  ...
  free(XSPPIN_DATA_BLOCKS(policy_data));

  /* Free valid head buckets */
  if (XSPPIN_DATA_VALID_HEAD(policy_data))
  {
    free(XSPPIN_DATA_VALID_HEAD(policy_data));
  }

  /* Free valid tail buckets */
  if (XSPPIN_DATA_VALID_TAIL(policy_data))
  {
    free(XSPPIN_DATA_VALID_TAIL(policy_data));
  }
}
    ...
    xsppin_gdata *global_data, long long tag, memory_trace *info)
{
  ...
  {
    head = XSPPIN_DATA_VALID_HEAD(policy_data)[rrpv].head;
...
void cache_fill_block_xsppin(xsppin_data *policy_data, xsppin_gdata *global_data, 
    ...
    memory_trace *info)
{
  ...
  block = &(XSPPIN_DATA_BLOCKS(policy_data)[way]);
  ...
  if (info)
  {
    rrpv = cache_get_fill_rrpv_xsppin(policy_data, global_data, info, NULL);

    if (info->stream == CS)
    {
      add_region_block(CS, info->stream, info->vtl_addr);
      rrpv = cache_get_fill_rrpv_xsppin(policy_data, global_data, info, &(block->is_ct_block));
    ...
    if (info->stream == BS)
    {
      ...
      rrpv = cache_get_fill_rrpv_xsppin(policy_data, global_data, info, &(block->is_bt_block));
    ...
    if (info->stream == ZS)
    {
      ...
      rrpv = cache_get_fill_rrpv_xsppin(policy_data, global_data, info, &(block->is_zt_block));
  ...
  {
    rrpv = XSPPIN_DATA_MAX_RRPV(policy_data);
  }
  ...
  if (rrpv != BYPASS_RRPV)
  {
    ...
    assert(rrpv <= XSPPIN_DATA_MAX_RRPV(policy_data));
    global_data->per_stream_frrpv[info->stream][rrpv] += 1;

#if 0
    block->access = (XSPPIN_DATA_MAX_RRPV(policy_data) - rrpv);

    /* Modulate RRPV based on fill count */
#define NUM(p, i)   ((p->max_fills - p->fills[i->stream]) * XSPPIN_DATA_MAX_RRPV(p))
#define DNM(p)      (p->max_fills - p->min_fills)

    if (rrpv != XSPPIN_DATA_MAX_RRPV(policy_data))
    {
...
#undef DNM
    ...
    CACHE_APPEND_TO_QUEUE(block, 
        XSPPIN_DATA_VALID_HEAD(policy_data)[rrpv], 
        XSPPIN_DATA_VALID_TAIL(policy_data)[rrpv]);
  }

  XSPPIN_DATA_CFPOLICY(policy_data) = XSPPIN_DATA_DFPOLICY(policy_data);
}
...
int cache_replace_block_xsppin(xsppin_data *policy_data, xsppin_gdata *global_data)
{
  ...
  for (block = XSPPIN_DATA_FREE_HEAD(policy_data); block; block = block->prev)
  {
  ...
  rrpv = cache_get_replacement_rrpv_xsppin(policy_data);

  /* Ensure rrpv is with in max_rrpv bound */
  assert(rrpv >= 0 && rrpv <= XSPPIN_DATA_MAX_RRPV(policy_data));

  if (min_wayid == ~(0))
  {
    /* If there is no block with required RRPV, increment RRPV of all the blocks
     * until we get one with the required RRPV */
    if (XSPPIN_DATA_VALID_HEAD(policy_data)[rrpv].head == NULL)
    {
      CACHE_XSPPIN_INCREMENT_RRPV(XSPPIN_DATA_VALID_HEAD(policy_data), 
          XSPPIN_DATA_VALID_TAIL(policy_data), rrpv);
    }

#define XSTRM_BLOCK(b)  ((b)->is_bt_block == TRUE || (b)->is_ct_block == TRUE || (b)->is_zt_block == TRUE)

    switch (XSPPIN_DATA_CRPOLICY(policy_data))
    {
      case cache_policy_xsppin:

        for (block = XSPPIN_DATA_VALID_TAIL(policy_data)[rrpv].head; block; block = block->next)
        {
...
#undef XSTRM_BLOCK
  ...
  if (min_wayid != ~(0))
  {
    block = &(XSPPIN_DATA_BLOCKS(policy_data)[min_wayid]);
...
void cache_access_block_xsppin(xsppin_data *policy_data, xsppin_gdata *global_data, int way, 
    int strm, memory_trace *info)
{
  ...
  blk  = &(XSPPIN_DATA_BLOCKS(policy_data)[way]);
  prev = blk->prev;
  next = blk->next;
  ...
  switch (XSPPIN_DATA_CAPOLICY(policy_data))
  {
    case cache_policy_xsppin:
      ...
      if (info && info->fill == TRUE)
        ...
        if (info)
        {
          ...
          new_rrpv = cache_get_new_rrpv_xsppin(policy_data, global_data, old_rrpv, info);

          /* Update block queue if block got new RRPV */
          if (new_rrpv != old_rrpv)
          {
            CACHE_REMOVE_FROM_QUEUE(blk, XSPPIN_DATA_VALID_HEAD(policy_data)[old_rrpv],
                XSPPIN_DATA_VALID_TAIL(policy_data)[old_rrpv]);
            CACHE_APPEND_TO_QUEUE(blk, XSPPIN_DATA_VALID_HEAD(policy_data)[new_rrpv], 
                XSPPIN_DATA_VALID_TAIL(policy_data)[new_rrpv]);
          }
    ...
    xsppin_gdata *global_data, memory_trace *info, ub1 *is_x_block)
{
  ...
  new_rrpv = XSPPIN_DATA_MAX_RRPV(policy_data) - 1;  

  switch (XSPPIN_DATA_CFPOLICY(policy_data))
  {
    case cache_policy_lru:
      return 0;

    case cache_policy_lip:
      return XSPPIN_DATA_MAX_RRPV(policy_data);
    ...
    case cache_policy_xsppin:
      if (info->stream == CS)
      {
        ...
        if (tmp_rrpv != XSPPIN_DATA_MAX_RRPV(policy_data))
        {
          new_rrpv = x_rrpv;
...
int cache_get_new_rrpv_xsppin(xsppin_data *policy_data, xsppin_gdata *global_data, int old_rrpv, 
    memory_trace *info)
{
  int new_rrpv;
  int reuse_ratio;
  ...
  enum cache_block_state_t state, ub1 stream, memory_trace *info)
{
  ...
  CACHE_REMOVE_FROM_QUEUE(block, XSPPIN_DATA_VALID_HEAD(policy_data)[old_rrpv],
      XSPPIN_DATA_VALID_TAIL(policy_data)[old_rrpv]);
  CACHE_APPEND_TO_SQUEUE(block, XSPPIN_DATA_FREE_HEAD(policy_data), 
      XSPPIN_DATA_FREE_TAIL(policy_data));
}
  ...
  long long *tag_ptr, enum cache_block_state_t *state_ptr, int *stream_ptr)
{
  ...
  PTR_ASSIGN(tag_ptr, (XSPPIN_DATA_BLOCKS(policy_data)[way]).tag);
  PTR_ASSIGN(state_ptr, (XSPPIN_DATA_BLOCKS(policy_data)[way]).state);
  PTR_ASSIGN(stream_ptr, (XSPPIN_DATA_BLOCKS(policy_data)[way]).stream);

  return XSPPIN_DATA_BLOCKS(policy_data)[way];
}
...
void cache_set_current_fill_policy_xsppin(xsppin_data *policy_data, cache_policy_t policy)
{
  ...
  XSPPIN_DATA_CFPOLICY(policy_data) = policy;
}
...
void cache_set_current_access_policy_xsppin(xsppin_data *policy_data, cache_policy_t policy)
{
  ...
  XSPPIN_DATA_CAPOLICY(policy_data) = policy;
}
...
void cache_set_current_replacement_policy_xsppin(xsppin_data *policy_data, cache_policy_t policy)
{
  ...
  XSPPIN_DATA_CRPOLICY(policy_data) = policy;
}
