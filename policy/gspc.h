...
#ifndef MEM_SYSTEM_GSPC_H
#define	MEM_SYSTEM_GSPC_H
...
typedef struct cache_list_head_gspc_t
{
  ub4 rrpv;
  struct cache_block_t *head;
}gspc_list;

#define GSPC_DATA_MAX_RRPV(data)   ((data)->max_rrpv)
#define GSPC_DATA_THRESHOLD(data)  ((data)->threshold)
#define GSPC_DATA_BLOCKS(data)     ((data)->blocks)
#define GSPC_DATA_VALID_HEAD(data) ((data)->valid_head)
#define GSPC_DATA_VALID_TAIL(data) ((data)->valid_tail)
#define GSPC_DATA_FREE_HEAD(data)  ((data)->free_head)
#define GSPC_DATA_FREE_TAIL(data)  ((data)->free_tail)

/* RRIP specific data */
typedef struct cache_policy_gspc_data_t
{
  cache_policy_t  following;                    /* Policy set is following */

  ub4        max_rrpv;                          /* Maximum RRPV */
  ub4        threshold;                         /* Eviction threshold */
  gspc_list *valid_head;                        /* Valid block head */
  gspc_list *valid_tail;                        /* Valid block tail */
...
typedef struct cache_policy_gspc_gdata_t
{
...
void cache_init_gspc(long long int set_indx, struct cache_params *params, 
  gspc_data *policy_data, gspc_gdata *global_data);
...
void cache_access_block_gspc(gspc_data *policy_data, gspc_gdata *global_data,
...
struct cache_block_t cache_get_block_gspc(gspc_data *policy_data, gspc_gdata *global_data, 
...
int cache_get_new_rrpv_gspc(gspc_data *policy_data, gspc_gdata *global_data, 
  int way, int strm);
...
void cache_update_fill_counter_gspc(srrip_data *policy_data, 
  gspc_gdata *global_data, int way, int strm);
...
void cache_update_hit_counter_gspc(srrip_data *policy_data,
  gspc_gdata *global_data, int way, int strm);
